...
public struct RefreshTokenModelMacro: MemberMacro {
    public static func expansion(of node: SwiftSyntax.AttributeSyntax, providingMembersOf declaration: some SwiftSyntax.DeclGroupSyntax, in context: some SwiftSyntaxMacros.MacroExpansionContext) throws -> [SwiftSyntax.DeclSyntax] {
        ...
        return [
            ...
            """
            public static func findByID(id: UUID, on database: Database) async throws -> \(raw: name)? {
                let refreshToken = try await \(raw: name).query(on: database).withDeleted().filter(\\.$id == id).with(\\.$accessToken, withDeleted: true).with(\\.$user).with(\\.$client).with(\\.$scopes).first()
                return refreshToken
            }
            """,
        ]
